...
<header class="bg-w">
	<span>Landlord's Supply - Water</span>
	<ul>
		...
		</li><?php }; if($w_meter) { ?><li><a href="#" data-view="building_supply_water?building_id=<?= $building_id; ?>" class="bg-w active"><i class="eticon eticon-shadow eticon-droplet"></i></a></li><?php } ?>
	</ul>
</header>
...
<?php
	...
	$meter = $w_meter ? new Meter($w_meter->id) : null;
...
<script>
	$(function() {
		...
		var getTariffDetails = function() {
			...
			$.getJSON('<?= APP_URL ?>/ajax/get/tariff_water_info?tariff_id=' + id, function(result) {
		...
		$('#update').on('click', function(e) {
			...
			$.post('<?= APP_URL ;?>/ajax/post/update_landlord_supply', $form.serialize(), function(data) {
				$.ajaxResult(data, function() {
					$.messagebox('Water supply updated.', {
		...
		var addTariff = function(e) {
			...
			$.ajaxModal('<?= APP_URL ?>/ajax/modals/crud-tariff-water.php?client_id=<?= $building->info->client_id; ?>&mpan=' + getMpan(), { size: 'lg' });
		};

		var editTariff = function(el) {
			var id = $(el).data('id');
			$.ajaxModal('<?= APP_URL ?>/ajax/modals/crud-tariff-water.php?client_id=<?= $building->info->client_id; ?>&mpan=' + getMpan() + '&tariff_id=' + id, { size: 'lg' });
		};
