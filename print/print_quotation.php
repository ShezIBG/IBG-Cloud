	...
	function render_summary_itemised(&$page) {
		...
		if(self::$hide_labour) {
			...
			$hardware = App::sql()->query(
				...
				WHERE ln.project_id = '$project_id'
				GROUP BY p.id, p.manufacturer_id, p.model, p.sku, p.image_id, COALESCE(p.short_description, ln.description), ln.unit_price, uc.path
				ORDER BY p.sku;
			", MySQL::QUERY_ASSOC) ?: [];
		} else {
			// With labour
			$hardware = App::sql()->query(
				...
				WHERE ln.project_id = '$project_id'
				GROUP BY p.id, p.manufacturer_id, p.model, p.sku, p.image_id, COALESCE(p.short_description, ln.description), uc.path
				ORDER BY p.sku;
			", MySQL::QUERY_ASSOC) ?: [];
		}
								...
								<div style="margin-top: 30px;">
									<table class="hardware" style="width: 100%;">
										<thead class="text-m">
											<tr>
												<th></th>
												<th>Model / Description / SKU</th>
												<th class="text-right">QTY</th>
												<?php if(self::$hide_labour) { ?>
													...
													?>
														<tr>
															...
															<td>
																...
																<div style="font-size: 0.8em;"><?= $row['sku'] ?></div>
															</td>
															<td class="text-right"><?= $row['quantity'] ?></td>
															<?php if(self::$hide_labour) { ?>
																<td class="text-right">&pound;<?= App::format_number_sep($row['unit_price'], 2, 2) ?></td>
															<?php } ?>
															<td class="text-right shrink" style="width: <?= $col_subtotal_width ?>;">&pound;<?= App::format_number_sep($row['total'], 2, 2) ?></td>
														</tr>
													<?php
												}
											?>
