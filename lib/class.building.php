...
class Building {
	...
	public function API_get_floors($filter = [], $fields = ['floor.*'], $joins = [], $api_building_id) {
		return App::sql()->query("SELECT ".implode(', ', $fields)." FROM floor ".implode(' ', $joins)." WHERE floor.building_id = $api_building_id ".App::sql()->build_filter_string($filter)." ORDER BY floor.display_order");
	}
	...
	public function get_mmm_meter_readings_html($meter_id, $partial = false) {
		...
		$color = '#4F81A0';
		switch($meter_type) {
			case 'E':  $icon_class = 'eticon eticon-bolt txt-color-yellow';        $color = '#56b87c '; break;
			...
			case 'EG': $icon_class = 'eticon eticon-leaf txt-color-green';         $color = '#A2B83A'; break;
		}
		...
		//SELECT building_id FROM meter WHERE id = 926;
		$building_id = App::sql()->query("SELECT building_id FROM meter WHERE id = '$meter->id';",MySQL::QUERY_ASSOC, false);
			
		if($building_id[0]['building_id'] == 129 || $building_id[0]['building_id'] == 80){
			$yday = date('Y-m-d', strtotime("-1 days"));
			$content .= "<h2>Hourly Data: <input type='date' name='selected_date' id='selected_date' value='$yday'></h2>";
			//$content .= "<h2>Hourly Data: <input type='text' name='selected_date' id='selected_date' value='$yday'><button class='btn btn-default toggle-partial pull-right' id='show-date-button' type='button'><i class='eticon eticon-calendar'></i> Select Date</button></h2>";
			$content .= "<canvas id='myChart' style='width:100%;height:30vh;max-height:400px;'></canvas>";

			// The script for updating the graph with new data
			$content .= '<script>
				var chart; // Declare the chart instance variable
				var meter_id = ' . json_encode($meter->id) . '
				function updateChart(selectedDate) {
					// AJAX call to fetch new data based on the selected date
					$.ajax({
						type: "POST",
						url: "'.APP_URL.'/lib/hourly.graph.update.php", // Replace with the URL of your PHP script that handles the SQL query
										...
										scales: {
											x: {
												...
												title: {
													...
													font: {
														size: 11,
														family: "Poppins, Sansation,Helvetica Neue,Helvetica,Arial,sans-serif"
													}
												},
											},
											y: {
				...
				// Call the updateChart function when the selected_date input changes
		...
		if (!empty($replacement_reading[0]['replaced_meter_reading'])){
			$content .= '<div><h2 style="display:inline-block;">Replacement meter readings</h2></div>';
			$content .= '<div>
	...
	public function get_mmm_building_summary_html() {
		...
		$meters_table
			->cell('description', function($row, $value) {
				$desc = '';
				if($row->sub_meter) $desc = '&nbsp;<i class="eticon eticon-arrow-up" style="color: #ccc;"></i>&nbsp;&nbsp;&nbsp;';
				$icon_class = '';
	...
	public function API_get_area_ids_with_permission($permission, $building_id) {
		$areas = Permission::list_areas([ 'with' => $permission, 'filter_level' => PermissionLevel::BUILDING, 'id' => $building_id ]);
		if($areas) $areas = array_map(function($a) { return $a->area_id; }, $areas);
		return $areas;
	}
