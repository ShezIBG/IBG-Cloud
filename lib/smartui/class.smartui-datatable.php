	...
	public function print_js($return = false, $jquery_enclosed = true) {
		...
		if ($structure->options["static"]) {
			if ($structure->options['row_details_opened']) {
				$row_detail_code = parent::is_closure($structure->options['row_details_opened']) ? $structure->options['row_details_opened']($table_id) : $structure->options['row_details_opened'];
			} else {
			...
			$dtable_js = '
				...
				$("#'.$table_id.' a i[data-toggle=\'row-detail\']").on("click", function (e) {
					...
					if (details.length > 0) {
						...
						$(this).removeClass("'.SmartUI::$icon_source.'-'.$this->options['row_detail_icons']['opened'].'").addClass("'.SmartUI::$icon_source.'-'.$this->options['row_detail_icons']['closed'].'");
						this.title = "Show Details";

						details.remove();
					} else {
						/* Open this row */
						$(this).removeClass("'.SmartUI::$icon_source.'-'.$this->options['row_detail_icons']['closed'].'").addClass("'.SmartUI::$icon_source.'-'.$this->options['row_detail_icons']['opened'].'");
			...
			$dtable_js = '
				...
				function tooltip_placement(context, source) {
					...
					if( Math.floor(off2.left) < Math.floor(off1.left) + Math.floor(w1 / 2) ) return "right";
					return "left";
				}
				...
				// Save datatable reference in jQuery data attribute
				$("#'.$table_id.'").data("dt", '.$otable_var.');
			...
			"if_array" => function($data) use ($that, $get_property_value, $structure) {
				...
				foreach ($data as $row_index => $row_data) {
					...
					if (isset($structure->options["row_details"]) && $structure->options["row_details"]) {
						...
						$detail_prop = array(
							"id" => "",
							"icon" => SmartUI::$icon_source."-".$this->options["row_detail_icons"]["closed"],
							"title" => 'Show Details'
						);
						...
						$content = '<a href="#" '.$id.'>
									<i class="'.SmartUI::$icon_source.' '.$detail_prop['icon'].' '.SmartUI::$icon_source.'" data-toggle="row-detail" title="'.$detail_prop['title'].'"></i>
								</a>';
