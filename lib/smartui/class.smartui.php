...
class SmartUI extends SmartUtil {
	...
	public static function print_alert($message, $type = 'info', $options = array(), $return = false) {
		...
		if ($new_options_map['icon']) {
			$icon = (isset($icon_map[$new_options_map['icon']]) ? $icon_map[$new_options_map['icon']] : SmartUI::$icon_source.'-'.$new_options_map['icon']);
			$icon_html = '<i class="'.SmartUI::$icon_source.' '.$icon.'"></i>';
		} else $icon_html = '';
