<?php
	...
	if ($building = $user->get_default_building(Permission::ELECTRICITY_ENABLED)) {
		...
		$content = '
			<p class="myWidget-midMain-title">Top Consuming Categories</p>
			<div class="widget-row">
				'.$ui_table->print_html(true).'
			</div>
			<hr class="myLine2"/><div class="dashboard-widget-content-separator"><p class="myWidget-title2">How you\'ve used your power</p></div>
			<div class="widget-row widget-row-chart display-flex no-flex">
				<div class="chart chart_second" id="your-electricity-chart"></div>
				<div>
		...
		$ui_widget->header('title', '
		 <p class="myWidget-title">Your Electricity</p><i class="myWidget-colorIcon eticon eticon-bolt eticon-bolt-color eticon-shadow"></i>
			<div class="clearfix"></div>
			<div class="widget-row display-flex no-flex">
				<div>
					<div class="myWidget-title-total">Power Used</div>
					<div class="myWidget-colorTotal padding-top-5">'.$total_kwh.' kWh</div>
				</div>
				<div>
					<div class="myWidget-title-total">Cost</div>
					<div class="myWidget-colorTotal padding-top-5">&pound;'.number_format($total_cost, 2).'</div>
				</div>
			</div>');
