<?php
	...
	if ($building = $user->get_default_building(Permission::WATER_ENABLED)) {
		...
		$sub_meters = App::sql()->query("SELECT m.id, a.description AS area FROM meter AS m JOIN area AS a ON a.id = m.area_id JOIN floor AS f ON f.id = a.floor_id WHERE f.building_id = '$building->id' AND m.meter_type = 'W' AND m.parent_id IS NOT NULL ORDER BY m.description;");
		...
		if($sub_meters) {
			...
			foreach($sub_meters as $m) {
				...
				$table_data[] = [
					...
					'Usage' => $usage_total.' '.$meter->get_reading_unit(true)
				];
			}
		...
		$ui_widget->header('title', '<p class="myWidget-title">Sub-meters</p><i class="mmyWidget-colorIcon eticon eticon-droplet-color eticon-droplet eticon-shadow" style="margin-top: 15px !important;"></i>');
		$ui_widget->footer = ' ';
...
<script>
	$(function() {
		var $w = $('#sub-meter-list-widget');

		$w.find('table tbody tr:eq(0)').addClass('bg-color-tealLighter');

		$w.find('table').on('click', 'tbody tr', function() {
			var $this = $(this);

			$w.find('table .bg-color-tealLighter').removeClass('bg-color-tealLighter');
			$this.addClass('bg-color-tealLighter');

			var id = ($this.find('td:eq(0)').text() || '').trim();
			loadWidget($('#sub-meter-detail-widget').closest('.grid-stack-item'), {
