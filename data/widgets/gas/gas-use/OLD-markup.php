...
<?php
	...
	$ui_widget->color = 'blue';
	$ui_widget->header('title', '<p class="myWidget-title">Your Gas Use</p><i class="myWidget-colorIcon eticon eticon-meter eticon-shadow"></i>');
		...
		if($single_day) {
			...
			foreach($gas_meters as $m) {
				...
				if($meter->validate($building->id)) {
					$usage_array[] = $meter->get_hourly_usage($time_period->date_from);
				}
			}
			...
			if(isset($usage[$time_period->date_from])) {
				...
				$chart_data = [];
				$tick = 0;
				$tickSize = count($usage) > 10 ? 2 : 1;

				foreach($usage as $hour => $data) {
					$rgb = $working_hours[$hour] ? '49,63,76' : '189,189,189'; 
			...
			if($usage) {
				...
				foreach($usage as $day => $data) {
					...
					$rgb = $working_days[$day] ? '49,63,76' : '189,189,189';
