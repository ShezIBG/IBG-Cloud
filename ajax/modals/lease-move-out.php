...
<div class="modal-header">
	...
	<h4 class="modal-title txt-color-red">Move out</h4>
</div>
...
<?php
	...
	if (!$building_id) {
		...
		if(!$lease->is_current()) {
			...
			$form->id = 'lease-move-out-form';

			$form->footer(function() use ($ui) {
				return implode(' ', [
					$ui->create_button('Move Out', 'danger')->attr([ 'type' => 'submit' ])->print_html(true),
	...
	var runFunction = function() {
		...
		$form.validate({
			...
			submitHandler: function() {
				$.post('<?= APP_URL.'/ajax/post/lease_move_out'; ?>', $form.serialize(), function(data) {
