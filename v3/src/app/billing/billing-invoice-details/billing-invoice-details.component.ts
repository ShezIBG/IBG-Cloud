...
@Component({
	...
	styleUrls: ['./billing-invoice-details.component.less']
})
export class BillingInvoiceDetailsComponent implements OnInit, OnDestroy {
	...
	refresh() {
		this.disabled = true;
		this.api.billing.getInvoice(this.id, response => {
			this.disabled = false;
				...
				this.app.header.addButton({
					...
					callback: () => {
						if (confirm('Are you sure you want to re-send the invoice to the customer?')) {
							this.api.billing.resendInvoiceEmail(this.id, () => {
	...
	approveInvoice() {
		if (confirm('Are you sure you want to approve this invoice?')) {
			this.disabled = true;
			this.api.billing.approveInvoice(this.id, _ => {
	...
	setInvoiceStatus(status) {
		...
		this.disabled = true;
		this.api.billing.setInvoiceStatus(this.id, status, () => {
	...
	editInvoiceNo() {
		...
		this.app.modal.open(BillingInvoiceNoEditModalComponent, this.moduleRef, {
			invoice_id: this.id
		});
	}
