...
@Component({
	selector: 'app-billing-system-integrator-details',
	templateUrl: './billing-system-integrator-details.component.html'
})
export class BillingSystemIntegratorDetailsComponent implements OnInit, OnDestroy {
	...
	refresh() {
		this.api.billing.getSystemIntegrator(this.owner, this.id, response => {
			if (!this.destroyed) {
				...
				const baseRoute = '/billing/' + this.owner + '/system-integrator/' + this.id;
	...
	createPaymentAccount() {
		if (confirm('Are you sure you want to create a payment account for this system integrator?')) {
			this.api.billing.createSystemIntegratorAccount(this.owner, this.id, () => {
				this.refresh();
			}, response => {
				this.app.notifications.showDanger(response.message);
			});
