...
@Component({
	selector: 'app-stock-goods-out',
	templateUrl: './stock-goods-out.component.html',
	styleUrls: ['./stock-goods-out.component.less']
})
export class StockGoodsOutComponent implements OnInit, OnDestroy {
	...
	refresh() {
		this.api.stock.listWarehouses(this.app.selectedProductOwner, response => {
			...
			this.app.header.addCrumb({ description: 'Goods Out' });
		}, response => {
	...
	addProduct(product) {
		...
		this.api.stock.getStockProductInfo(product.id, this.warehouse, response => {
			...
			if (p.stock.length === 0) {
				this.app.notifications.showDanger('Product not in stock.');
				return;
			}

			p.quantity = 1;
			p.selected = p.stock[0];
			p.stock.forEach(s => {
				if (s.location_id === p.location_id) p.selected = s;
			});
	...
	submit() {
		...
		this.products.forEach(item => {
			items.push({
				product_id: item.id,
				location_id: item.selected.location_id,
				quantity: item.quantity
			});
		});

		this.api.stock.submitGoodsOut({
			warehouse_id: this.warehouse,
			notes: this.notes,
			items: items
		}, () => {
			this.app.notifications.showSuccess('Goods out submitted.');
			this.resetForm();
		}, response => {
			this.app.notifications.showDanger(response.message);
		});
