...
@Component({
	selector: 'app-stock-view-products',
	templateUrl: './stock-view-products.component.html',
	styleUrls: ['./stock-view-products.component.less']
})
export class StockViewProductsComponent implements OnInit, OnDestroy {
	...
	refresh() {
		this.api.stock.listWarehouses(this.app.selectedProductOwner, response => {
			...
			this.app.header.addCrumb({ description: 'Stock By Product' });
		}, response => {
	...
	refreshList() {
		...
		this.api.stock.listStockByProduct(this.warehouse, response => {
