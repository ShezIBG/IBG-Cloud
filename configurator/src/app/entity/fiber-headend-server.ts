...
export class FiberHeadendServer extends Entity implements MovableEntity, ActivatableEntity {

	static type = EntityTypes.FiberHeadendServer;
	static groupName = 'Headend Servers';

	get isActive() { return this.data.active === 1; }
	set isActive(value) { this.data.active = value ? 1 : 0; }

	getTypeDescription() { return 'Headend Server'; }
	...
	assignTo(entity: Entity) {
		if (EntityTypes.isRouter(entity)) {
			...
			this.data.ext_ip_address = entity.data.ip_address;
			this.refresh();
		}
	}
